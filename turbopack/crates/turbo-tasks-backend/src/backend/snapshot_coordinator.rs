...
///
/// This is a throughput measure, not a correctness one. A blocked operation waits here for the
/// snapshot/GC pass to release its exclusion, and that pass runs *on this runtime*; parking a
/// worker for the duration of a long pass starves everything else that could have used it. Liveness
/// does not depend on the hand-off: `scope_unbounded`'s join never waits on a helper the scheduler
/// has not polled, so the pass completes even with every other worker parked here.
///
...
impl<O> SnapshotCoordinator<O> {
    ...
    pub fn suspend_point(&self, suspend: impl FnOnce() -> O) {
        ...
        fn suspend_point_cold<O>(this: &SnapshotCoordinator<O>, suspend: impl FnOnce() -> O) {
            ...
            // so the runtime can use it meanwhile; see `block_off_runtime`.
            block_off_runtime(|| {
                this.exclusion_completed
                    .wait_while(&mut state, |s| s.snapshot_requested || s.gc_requested);
            });
